...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    expected = [rg.Point(13.0,7.0),  # All numbers are approximate.
                rg.Point(-0.5,14.8),
                rg.Point(-0.5,-0.8),]

    circle = rg.Circle(rg.Point(4, 7), 9)
...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    title = 'Test 4:  5 pink dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(250, 200), 90)
    draw_points_on_circle(window, circle, 50, 'pink')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    title = 'PIZZA test 4:  70 slices, thin (thickness=3) yellow lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    start = circle.center
    end = generate_points_on_circle(circle,number_of_slices)
    for k in range(number_of_slices):
        line = rg.Line(start,end[k])
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON test 3: 7 segments with thick pink lines'
    window = rg.RoseWindow(700, 700, title)
    circle = rg.Circle(rg.Point(350, 350), 50)
    circle.outline_thickness = 8
    polygon(window, circle, 7, 'pink', 8)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    start = generate_points_on_circle(circle,number_of_segments)
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(350, 350), 150)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    """
    # ------------------------------------------------------------------
    # DONEA: 10. Implement and test this function.
    #   Note that you should write its TEST function first (above).
    #
    # IMPLEMENTATION REQUIREMENT:
    #    You MUST USE (call) the   generate_points_on_circle
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    start = generate_points_on_circle(circle,number_of_lines)
    for k in range(number_of_lines):

        if k+hops_to_next_point>=number_of_lines:
            end = start[(k+hops_to_next_point)%number_of_lines]
        else:
            end = start[k+hops_to_next_point]
        line = rg.Line(start[k],end)
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
